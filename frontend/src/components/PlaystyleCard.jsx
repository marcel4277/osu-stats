...
const TRAITS = {
  hidden:     { label: 'HD Player',      title: 'Uses Hidden on the majority of plays',         style: 'bg-slate-700   text-slate-200'   },
  hrReader:   { label: 'HR Reader',      title: 'Regularly reads Hard Rock approach rates',     style: 'bg-rose-900    text-rose-300'    },
  dtLover:    { label: 'DT Enjoyer',     title: 'Frequently uses Double Time',                  style: 'bg-yellow-900  text-yellow-300'  },
  purist:     { label: 'Nomod Purist',   title: 'Plays almost exclusively without mods',        style: 'bg-sky-900     text-sky-300'     },
  accMachine: { label: 'Acc Machine',    title: 'Maintains unusually high average accuracy',    style: 'bg-emerald-900 text-emerald-300' },
  modStacker: { label: 'Mod Stacker',    title: 'Frequently combines multiple difficulty mods', style: 'bg-purple-900  text-purple-300'  },
  bareHands:  { label: 'No Mods At All', title: 'Rarely if ever touches HD, HR, or DT',        style: 'bg-gray-700    text-gray-300'    },
};
...
function analyse(scores) {
  ...
  const isNomod = s => !hasDT(s) && !hasHR(s);   // NM or NM+HD
  ...
  const bareCount = scores.filter(s => s.mods.length === 0).length; // truly no mods
  ...
  // Primary archetype — order matters, first match wins
  let key;
  ...
  // Mod breakdown bars (NM = no difficulty mods, regardless of HD)
  const breakdown = [
